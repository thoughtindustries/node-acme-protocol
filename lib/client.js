...
function Client( options ) {
  ...
  this.registration = new ACME.Registration()
...
Client.prototype = {
  ...
  register: function( contact, callback ) {
    ...
    registration.resource = ACME.RESOURCE.NEW_REGISTRATION // necessary?
    registration.contact = contact.slice()
  ...
  /**
   * Deactivate a registered account
   * @see ACME Draft 04, Section 6.3.3: Account deactivation
   * @param {ACME.Registration} registration
   * @param {Function} callback
   */
  deactivateRegistration: function( registration, callback ) {

    var self = this

    if( !(registration instanceof ACME.Registration) )
      registration = ACME.Registration.fromJSON( registration )

    // A client may deactivate an account by posting a signed update to the
    // server with a status field of "deactivated."
    registration.status = ACME.Registration.STATUS.DEACTIVATED
    ...
    this._request({
      ...
      callback.call( self, error, registration )
